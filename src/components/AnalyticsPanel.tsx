...
export default function AnalyticsPanel({ visitors }: Props) {
  ...
  const { days, purposes, peakHours } = useMemo(() => {
    ...
    visitors.forEach(v => { const h = v.checkInTime.getHours(); hourMap[h] = (hourMap[h] ?? 0) + 1 })
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="card p-4">
        ...
        <div className="flex items-end gap-2 h-28">
          {days.data.map(({ label, count }) => (
            <div key={label} className="flex-1 flex flex-col items-center gap-1">
              <span className="text-xs font-semibold" style={{ color: 'var(--text-2)' }}>{count > 0 ? count : ''}</span>
              <div className="w-full rounded-t-md transition-all" style={{
                height: `${Math.max(count / days.max * 96, count > 0 ? 6 : 2)}px`,
                background: label === 'Today' ? 'linear-gradient(180deg,#3B82F6,#2563EB)' : 'var(--surface)',
        ...
        <p className="text-xs mt-3" style={{ color: 'var(--text-3)' }}>Total: {visitors.length} visitor{visitors.length !== 1 ? 's' : ''} on record</p>
      </div>
            ...
            {purposes.map(({ name, count, pct }) => (
              <div key={name}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm">{PURPOSE_EMOJI[name] ?? ''} {name}</span>
                  <span className="text-xs font-semibold" style={{ color: 'var(--text-2)' }}>
                    {count} <span style={{ color: 'var(--text-3)' }}>({pct}%)</span>
                  </span>
                </div>
                <div className="h-2 rounded-full overflow-hidden" style={{ background: 'var(--surface)' }}>
                  <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: PURPOSE_COLORS[name] ?? '#6B7280' }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="card p-4">
        ...
        <div className="flex items-end gap-0.5 h-14">
          {peakHours.map(({ h, count, pct }) => (
            <div key={h} title={`${h}:00 — ${count} visits`} className="flex-1 rounded-sm transition-all" style={{
              height: `${Math.max(pct, count > 0 ? 8 : 2)}%`,
              background: pct > 60 ? '#10B981' : pct > 30 ? '#3B82F6' : count > 0 ? 'var(--surface)' : 'var(--card)',
              border: '1px solid var(--border)',
            }} />
          ))}
        </div>
