    ...
    <div className="min-h-screen bg-zinc-50 font-sans">
      ...
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-zinc-200">
        <div className="max-w-screen-xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          {/* Controls */}
          <div className="flex items-center gap-2">
            {/* Admin link — visible on desktop only; mobile uses FAB */}
            <Link
              href="/admin"
              className="hidden sm:flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg border border-zinc-200 bg-zinc-50 text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100 transition-colors"
            >
            ...
            {/* Animation toggle — icon only on mobile */}
            <button
              onClick={() => {
                setAnimationEnabled((v) => !v);
                setAnimationKey((k) => k + 1);
              }}
              className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-2 rounded-lg border transition-all duration-150 ${
                animationEnabled
            ...
            >
              ...
              <span className="hidden sm:inline">{animationEnabled ? "Typewriter On" : "Typewriter Off"}</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <Link
        href="/admin"
        className="sm:hidden fixed bottom-6 right-5 z-40 w-14 h-14 rounded-full bg-indigo-600 shadow-lg flex items-center justify-center text-white hover:bg-indigo-700 active:bg-indigo-800 transition-colors"
        title="Add question"
      >
